...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Registro Bomberos') }}</div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="form-group row">
                            <label for="cuartel" class="col-md-4 col-form-label text-md-right">{{ __('Cuartel') }}</label>

                            <div class="col-md-6">
                                <select id="cuartel" type="select" class="form-control" name="cuartel" required>
                                    <option>Seleccionar</option>
                                    @foreach(Cuarteles::getCuarteles() as $c)
                                        <option value="{{$c->cuarid}}" {{$c->cuarid  == old('cuartel') ? 'selected': '' }}>{{$c->cuardes}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
